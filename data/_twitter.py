...
class TwitterAPI:
  ...
  def fetch_by_content(phrase: str, num: int, from_t: datetime, to_t=datetime.today().strftime('%Y-%m-%d'))-> pd.DataFrame:
    ...
    """
    tweets = []
    for i, tweet in enumerate(sntwitter.TwitterSearchScraper('{} since:{} until:{}'.format(phrase, from_t, to_t)).get_items()):
        if i> num:
            break
